...
import { proposeIrrelevantThemes } from "@/lib/ingest/purgeThemes";
...
export async function POST() {
  try {
    ...
    const { proposals, errors } = await proposeIrrelevantThemes(region.id);
    return NextResponse.json({ proposals, errors });
  } catch (err) {
    console.error("proposeIrrelevantThemes failed:", err);
