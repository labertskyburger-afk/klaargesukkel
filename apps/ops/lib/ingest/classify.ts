    ...
    input_schema: {
      ...
      properties: {
        action: {
          ...
          description: `'irrelevant' = general news, national/international politics, or anything not specifically about ${areaPhrase} — e.g. an article about immigration law, climate policy, or a crime/news story set in some other Cape Town suburb not listed above. Sharing a broad topic that residents of ${areaPhrase} might also happen to care about (crime, cost of living, load-shedding, etc.) is NOT enough on its own — the signal must actually be about this specific local market: it names one of the suburbs listed above, or is otherwise unambiguously specific to this area, not just Cape Town or South Africa in general. Watch specifically for name collisions with genuinely different places: "Durban" is a completely different city in KwaZulu-Natal, ~1,600km away — NOT the same place as "Durbanville" despite the similar name, and a Durban-only mention is irrelevant. Likewise "Stellenbosch" (the well-known Winelands town) is a different place from the tracked suburbs "Stellenberg"/"Stellenridge"/"Stellenryk" — don't let the shared "Stellen-" prefix make a Stellenbosch-only story look local. Read the actual place name carefully rather than pattern-matching on partial similarity. This distinction matters most for the broad RSS news sources, which pull an entire regional feed with no geographic filtering upstream — don't let topical or textual similarity substitute for genuine geographic relevance. Use 'irrelevant' whenever a signal doesn't represent someone specifically in ${areaPhrase} needing, wanting, or struggling to find something. Otherwise 'attach' to a matching existing theme or propose a 'new' one.`,
        },
        theme_id: {
          type: "string",
