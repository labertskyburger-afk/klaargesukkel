# Klaargesukkel — instructions for Codex
...
Albert also works on this business in Codex's Cowork mode (Codex Desktop) — that's where
...
apps/
  ...
  ops/     — Next.js 14 + Tailwind. Private internal-ops app, live at ops.klaargesukkel.com.
             ...
             screenshot → Codex vision extraction → de-identified Signal, processed
...
- Daily Vercel Cron (`/api/cron/ingest`, `apps/ops/vercel.json`) that self-seeds the region
  ...
  (Codex Haiku 4.5 — cheap/fast, right for this) via `lib/ingest/classify.ts`
...
- The weekly manual-capture upload flow at `/eyespy/capture` — screenshot → Codex Opus 5
...
both in sync if any of these change):
 ...
 — the dataset is confirmed
  ...
  to find (Codex's fetch tooling hit 404s on the ArcGIS Hub site). Once you have it, it
...
**Update from Codex (2026-08-02): the accounts/sync build described below is done.**
...
**What's still blocked on Albert** (dashboard actions Codex can't do — needs real
...
1. **Deploy `apps/ops` to Vercel.** New merged app (see "Current state" above) — Root
   ...
   the public hub. Build-tested clean by Codex 2026-07-30 (full login → nav →
...
- No sandbox that built this had npm registry access, so builds were never verified with a
  ...
  hub builds fine. `apps/ops` has since been build-tested by Codex (2026-07-30).
...
I am in the ideation phase of my own business. I want to create solutions for people, organizations and business's everyday issues, struggles and just things that needs help or fixing. May it be a ordering system, a whatsapp chatbot, a marathon training program or any of the solutions that I have built on claud so far. I bought two domains on godaddy, klaargesukkel.com and klaargesukkel.co.za so I also need help hosting and setting up all my solutions and products on these domains so it cab be reached from anywhere, I already have a github and vercel account, used in other claud projects
